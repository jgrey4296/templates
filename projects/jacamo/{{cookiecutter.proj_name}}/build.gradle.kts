...
sourceSets {
    main {
        java {
            srcDir("src/environments")
            srcDir("src/agents")
            srcDir("src/organisations")
            srcDir("src/kotlin")
            srcDir("src/_configs")
        }
        resources {
...
tasks.register<JavaExec>("buildJCMDeps") {
    ...
    args("src/_configs/simple.jcm", "--deps")
    classpath = sourceSets["main"].runtimeClasspath
}
        ...
        try {
            javaexec {
                ...
                if (gradle.startParameter.logLevel.toString().equals("DEBUG")) {
                    args("src/tests/tests.jcm", "--log-conf", "jason/templates/console-debug-logging.properties")
                } else if (gradle.startParameter.logLevel.toString().equals("INFO")) {
                    args("src/tests/tests.jcm", "--log-conf", "jason/templates/console-info-logging.properties")
                } else {
                    args("src/tests/tests.jcm", "--log-conf", "jason/templates/console-lifecycle-logging.properties")
                }
